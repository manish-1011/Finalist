  ...
  const parseBoldText = (text: string): React.ReactNode[] => {
    ...
    while ((match = boldRegex.exec(text)) !== null) {
      ...
      if (match.index > lastIndex) {
        ...
        if (beforeText) {
          elements.push(
            <LinkifyText key={`linkify-text-${keyCounter++}`} text={beforeText} />
          );
        }
      }
      
      // Add the bold text (with link detection)
      elements.push(
    ...
    if (lastIndex < text.length) {
      ...
      if (remainingText) {
        elements.push(
          <LinkifyText key={`linkify-text-${keyCounter++}`} text={remainingText} />
        );
      }
    }
    ...
    if (elements.length === 0) {
      elements.push(
        <LinkifyText key={`linkify-text-${keyCounter++}`} text={text} />
      );
    }
